...
- 2026-09-06：模板改克隆DemoSwc架构：create_swc_template整体拷贝核心子系统（比较/增益/限幅/切换骨架），端口按字典顺序重命名、标定常量补齐、库链接彻底断链、骨架算术块single化、Compare常量跟枚举输入；dd_apply_calibration/signal的update加try（裸奔硬失败）、calibration引用搜索下探子系统。新SWC ChrgCtrl一次跑通全链，TrqArb回归全绿。修过4处：子系统输出端口独立编号、Init空值守卫、find_system不进library link（FollowLinks）、库链接覆写改彻底断链。
